/***************************************
Program filename: wofdif.cpp (Wheel of Fortune)
Author: Joshua Michael Strozzi
Date: February 9th to February 19th 2017
Description: The game runs an augmented version of Wheel of fortune
Input: phrases to be guessed, letter guesses, number of rounds and players
Output: masked versions of phrases, money, and a winner
****************************************/
...
string ignore_sol (string, string &);
int num_123();

int main(){
	int rounds, players;
	int mon1=0, mon2=0, mon3=0;
	int mon1t=0, mon2t=0, mon3t=0;
...
bool solve(string phrase, string &masked){
	...
	solution =ignore_sol(phrase, masked);
	if (solution==phrase){
...
/*************************************
** Function: consonant
** Description: Checks if the input is not a vowel, or if it's already in the function
** Parameters: masked
** Pre-Conditions: Not all the letters have been revealed
** Post-Conditions: If correct, letter is revealed regardless of case
** Return: the next letter to be revealed, regardless of case
***************************************/
...
/*************************************
** Function: vowel
** Description: Checks if the letter is a vowel and if it has been revealed yet
** Parameters: the masked string
** Pre-Conditions: Not all the vowels have been revealed
** Post-Conditions: If correct, a vowel will be revealed and the user loses 10 dollars
** Return: The vowel that will be revealed
***************************************/
...
/*************************************
** Function: get_guessv
** Description: This function is for the user to attempt guessing a vowel after deciding to buy a vowel
** Parameters: phrase, masked and money
** Pre-Conditions: Not all the letters have been revealed
** Post-Conditions: If correct, one vowel has been added to masked and user loses 10 dollars
** Return: Whether or not the user failed
***************************************/
...
/*************************************
** Function: winner
** Description: This function compares the money of each player and determines a winner
** Parameters: mon1 mon2 and mon3
** Pre-Conditions: The players have played and earned money
** Post-Conditions: the game is over and a winner has been decided
** Return: nothing
***************************************/
...
/*************************************
** Function: ignore_sol
** Description: This function is for the user to attempt guessing the full hidden phrase and ignore the case of whatever letters are in it
** Parameters: The phrase that we need to guess
** Pre-Conditions: Not all the letters have been revealed
** Post-Conditions: If correct, the round ends
** Return: Whether or not the user failed
***************************************/
string ignore_sol(string phrase, string &masked){
	string solution;
	getline(cin, solution);
	for(int i=0;i<phrase.length(); i++){
		if(solution.at(i)==phrase.at(i) or solution.at(i)==phrase.at(i)+32 or solution.at(i)==phrase.at(i)-32){
			solution.at(i)=phrase.at(i);
		}
	}	
	return solution;
}
...
int num_123(){
	...
	getline(cin, nums);
	nums=nums.at(0);
	while(nums.at(0)!='1' and nums.at(0)!='2' and nums.at(0)!='3'){
		cout<<"Wrong input\n Try again: ";
		getline(cin, nums);
		nums=nums.at(0);
